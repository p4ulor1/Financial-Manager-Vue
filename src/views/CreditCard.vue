transactionTable<script setup>
  ...
  function editEffectiveInvoice(newMonthEffectiveInvoice) {
    ...
    if (currentMonthEffectiveInvoice === undefined) {
      const month = parseISODate(date.value).month;

      currentCreditCardYearInfos.yearEffectiveInvoices[month - 1].id = newMonthEffectiveInvoice.id
      currentCreditCardYearInfos.yearEffectiveInvoices[month - 1].value = newMonthEffectiveInvoice.value
    }
    else
